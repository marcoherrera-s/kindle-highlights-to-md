...
def sanitize_filename(filename):
    """
    Sanitizes filenames to remove or replace characters that might be problematic.
    """
...
def export_txt(clips):
    ...
    for book in clips:
        lines = []
        # Add the book title at the start of the list
        lines.append(f"# {book}\n")

        # Continue with the existing code to add highlights
        for pos in sorted(clips[book], key=int):
            lines.append(clips[book][pos])

        safe_book_title = sanitize_filename(book)
        filename = os.path.join(OUTPUT_DIR, f"{safe_book_title}.md")
